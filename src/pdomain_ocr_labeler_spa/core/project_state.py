...
- ``PageState`` — *placeholder* dataclass with just ``page_index``.
  The rich version (``pdomain_book_tools.ocr.page.Page`` object, dirty
...
class PageState:
    ...
    the underlying ``pdomain_book_tools.ocr.page.Page`` object, dirty flags,
    ...
    # dicts (includes ``source`` field not present on pdomain_book_tools type).
