...
Delegates to ``pdomain_book_tools.text.normalize.normalize_string`` when available.
When the module is absent (older pin), returns the input string unchanged and
sets ``is_available() == False`` so callers can show the "requires pdomain-book-tools
≥ X.Y.Z" message.
...
try:
    from pdomain_book_tools.text.normalize import (  # type: ignore[import-untyped]  # pyright: ignore[reportMissingImports]
...
def is_available() -> bool:
    """Return True if pdomain_book_tools.text.normalize is importable.

    Used by the OCRConfigModal to decide whether to enable the normalize
    toggle or show "Requires pdomain-book-tools ≥ X.Y.Z".
    """
    return _AVAILABLE
...
def normalize_string(text: str, profile: str = "ascii") -> str:
    ...
    When pdomain_book_tools.text.normalize is available, delegates to
