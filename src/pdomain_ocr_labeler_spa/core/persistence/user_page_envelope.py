...
- ``build_envelope(...)`` — the high-level constructor that takes a
  ...
  produces the envelope. Needs ``pdomain_book_tools.Page`` in scope; ships
...
class ProvenanceApp:
    ...
    Note: the SPA writes ``name="pdomain_ocr_labeler_spa"`` for new saves
    ...
    slice) is responsible for setting ``name="pdomain_ocr_labeler_spa"``
...
class ProvenanceToolchain:
    ...
    pdomain_book_tools: str = UNKNOWN_METADATA_VALUE
    opencv_python: str | None = None

    def to_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            "python": self.python,
            "pdomain_book_tools": self.pdomain_book_tools,
        }
...
class UserPagePayload:
    ...
    ``page`` is the verbatim ``pdomain_book_tools.ocr.page.Page.to_dict()``
    output. We hold it as a plain ``dict`` here so this module can be
    imported without pdomain_book_tools (slice 8b deferral pattern); the
...
_SPA_APP_NAME = "pdomain_ocr_labeler_spa"
...
def build_envelope(
    *,
    ...
    ----------
    page
        A ``pdomain_book_tools.ocr.page.Page`` (or any object exposing
    ...
    Notes
    -----
    - ``app.name`` is hardcoded to ``"pdomain_ocr_labeler_spa"`` (spec §3
    ...
    - ``app.version`` and ``toolchain.python`` / ``toolchain.pdomain_book_tools``
    ...
    return UserPageEnvelope(
        ...
        provenance=UserPageProvenance(
            ...
            app=ProvenanceApp(
                name=_SPA_APP_NAME,
                version=_safe_package_version("pdomain-ocr-labeler-spa"),
            ),
            toolchain=ProvenanceToolchain(
                python=_python_version(),
                pdomain_book_tools=_safe_package_version("pdomain-book-tools"),
            ),
            ocr=ocr_provenance,
        ),
