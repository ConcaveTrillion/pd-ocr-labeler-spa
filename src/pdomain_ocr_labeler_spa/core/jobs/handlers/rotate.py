...
2. Rotate the source image in-place via ``pdomain_book_tools.ocr.rotation.rotate_image``.
...
async def handle_rotate_page(runner: JobRunner, job: Job) -> None:
    ...
    # Full implementation: rotate image via pdomain_book_tools.ocr.rotation,
