...
``pdomain_book_tools`` domain class; deferring its import to ``TYPE_CHECKING``
keeps the Protocol importable in tests that don't need pdomain-book-tools
...
if TYPE_CHECKING:
    # Imported lazily — ``pdomain_book_tools.ocr.page.Page`` is the canonical
