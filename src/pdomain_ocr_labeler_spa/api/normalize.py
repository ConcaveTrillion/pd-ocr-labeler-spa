...
- Issue #261 — "When pdomain_book_tools.text.normalize unavailable
  (checked via GET /api/normalize/available): show 'Requires
  pdomain-book-tools ≥ X.Y.Z' and disable toggles."

Routes:
- ``GET /api/normalize/available`` — returns ``{"available": bool}``.
  ``true`` when ``pdomain_book_tools.text.normalize`` is importable (i.e.
  the installed pdomain-book-tools version exposes the normalize API).
