...
def _resolve_static_dir() -> Path | None:
    ...
    "pdomain_ocr_labeler_spa") / "static"`` returns a ``PosixPath`` we can
    use directly. Dev: the same path against the source tree resolves
    to ``src/pdomain_ocr_labeler_spa/static/``, also a ``Path``.
    ...
    *this* level keyed by ``(package_name, resource_name)`` — see
    ...
    """
    return _resolve_resource_dir("pdomain_ocr_labeler_spa", "static")
...
def install_spa_fallback(app: FastAPI) -> None:
    ...
    is copied into ``src/pdomain_ocr_labeler_spa/static/`` by ``make
    ...
    if static_dir is None:
        log.warning(
            ...
            "src/pdomain_ocr_labeler_spa/static/). Run `make frontend-build` "
        ...
        async def _spa_fallback_missing(full_path: str) -> Response:
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "to populate src/pdomain_ocr_labeler_spa/static/."
                ),
            )
