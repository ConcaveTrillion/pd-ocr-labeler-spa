...
- ``HF_LATEST_KEY`` (both keys equal ``"huggingface"``): download both
  weight files from ``HF_DEFAULT_REPO`` via ``pd_book_tools.hf.hf_download``.
...
def _resolve_hf_weights(hf_revision: str | None) -> ResolvedWeights | None:
    ...
    """
    try:
        hf_mod = importlib.import_module("pd_book_tools.hf")
    except ImportError:
        logger.debug("pd_book_tools.hf not available; HF weight resolution disabled")
        return None
