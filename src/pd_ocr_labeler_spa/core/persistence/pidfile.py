...
On startup the SPA writes a pidfile ``<cache_root>/pd-ocr-labeler-spa.pid``
...
_PIDFILE_NAME = "pd-ocr-labeler-spa.pid"


def pidfile_path(cache_root: Path) -> Path:
    """``<cache_root>/pd-ocr-labeler-spa.pid``."""
    ...
    if path.exists():
        try:
            ...
            if existing_pid != os.getpid() and _pid_is_alive(existing_pid):
                logger.warning(
                    "pd-ocr-labeler-spa startup: cache root %s appears to be held by "
