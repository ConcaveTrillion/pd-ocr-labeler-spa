...
def _load_page_from_envelope_file(json_path: Path) -> Any | None:
    ...
    Returns ``None`` on any error.  Import of ``pd_book_tools`` is deferred
    to keep this module importable even in test environments that stub
    ``pd_book_tools``.
    """
    try:
        from pd_book_tools.ocr.page import Page as PdPage
...
# ---------------------------------------------------------------------------
# Single-page export (sync, wraps pd-book-tools)
# ---------------------------------------------------------------------------
