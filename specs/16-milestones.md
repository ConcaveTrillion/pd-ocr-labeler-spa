...
- `make build` — produces a wheel; `python -m zipfile -l <whl>` shows
  `pd_ocr_labeler_spa/static/index.html`.
...
- `src/pd_ocr_labeler_spa/api/middleware/{error_handler,request_id,__init__}.py`
- `src/pd_ocr_labeler_spa/api/dependencies.py`
- `src/pd_ocr_labeler_spa/core/{app_state,logging_config,__init__}.py`
- `src/pd_ocr_labeler_spa/core/persistence/{paths,session_state,__init__}.py`
- `src/pd_ocr_labeler_spa/bootstrap.py` — full factory wiring
- `src/pd_ocr_labeler_spa/__main__.py` — CLI flags
...
**pd-book-tools** (`pd_book_tools.ocr.glyph_annotations` — NOT yet
...
- `src/pd_ocr_labeler_spa/core/glyph/predictions.py` — `IGlyphPredictor`
  Protocol; `none_` and `local_pdtrainer` adapters.
- `src/pd_ocr_labeler_spa/core/page_state.py` — `set_glyph_annotations`,
...
- `src/pd_ocr_labeler_spa/core/models.py` — extend `WordMatch` with
...
M6–M9 ≈ 1–2 sessions each (most logic is already in `pd-book-tools`
