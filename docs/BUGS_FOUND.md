...
- **Status:** **OPEN** (still). Decision landed 2026-05-07: option (A) with WARNING-level drift signal — `extra="ignore"` + `logger.warning("session_state_extras_dropped …")` so library/SPA-legacy drift in a release that shouldn't have had it surfaces as a grep-able log signal. See [D-041](../specs/17-decisions.md#d-041--session_statejson-extras-tolerance-with-warning-level-drift-signal). **Blocked-on: implementation per D-041** (NOT blocked on user decision — user has answered). Closeout requires: (1) flip `model_config` in `core/persistence/session_state.py`, (2) add the WARNING log with stable substring `session_state_extras_dropped`, (3) amend spec §6 with the explicit reader-tolerance + drift-signal clause and the §11 asymmetry note, (4) update the module docstring to cite §6 instead of §11, (5) pin with `test_session_state_load_ignores_unknown_keys` + `test_session_state_load_logs_warning_with_stable_substring`.
- **Severity:** medium
